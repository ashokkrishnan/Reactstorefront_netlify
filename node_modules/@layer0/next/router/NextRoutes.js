...
class NextRoutes extends PluginBase_1.default {
     ...
     */
    getPagesManifest() {
        return nonWebpackRequire_1.default(path_1.join(process.cwd(), this.distDir, 'serverless', 'pages-manifest.json'));
    }
    /**
    ...
    addPagesInProd(group) {
        ...
        const ssgRoutes = new Set(Object.entries(prerenderManifest.routes)
            .filter(([_route, config]) => config.srcRoute == null)
            .map(([route, _config]) => route));
        ...
        for (let page of pages) {
            ...
            if (page.startsWith('/api')) {
                ...
                if (this.defaultLocale && page.startsWith(`/${this.defaultLocale}`)) {
                    ...
                    addRoute('SSG html', nextPathFormatter_1.localize(locales, this.removeLocale(path)), this.createSSGHandler(assetPath.replace(new RegExp(this.defaultLocale + '(/|$)'), ':locale$1')));
                }
                ...
                if (dynamicRouteConfig || ssgRoutes.has(page)) {
                    ...
                    addRoute('SSG (λ) json', `/_next/data/:__build__${nextPathFormatter_1.localize(locales, nextPathFormatter_1.toRouteSyntax(page, { suffix: 'json' }))}`, this.createSSGHandler(page, {
                        dataRoute: true,
                        localize: localizationEnabled,
                    }));
                    // SSG: getStaticProps
                    addRoute(`SSG (λ${(dynamicRouteConfig === null || dynamicRouteConfig === void 0 ? void 0 : dynamicRouteConfig.fallback) ? ' w/fallback' : ''}) html`, nextPathFormatter_1.localize(locales, nextPathFormatter_1.toRouteSyntax(page)), this.createSSGHandler(page, {
                        localize: localizationEnabled,
                    }));
                }
                else {
     ...
     * @param options
     */
    createSSGHandler(page, { dataRoute, localize, } = {}) {
        return (res) => {
            ...
            if (dynamicRouteConfig) {
                ...
                const assetFile = nextPathFormatter_1.toRouteSyntax(page, { suffix }).replace(/\/\.(json|html)$/, '.$1'); // fix for :locale/index.js
                ...
                let assetPath = `${slash_1.default(path_1.join(assetRoot, page))}`;
